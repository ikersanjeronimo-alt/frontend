...
- **Notificaciones en vivo (toast):** `services/notificationsWS.ts` se suscribe a la cola personal `/user/queue/notifications`; los mensajes alimentan `store/notificationsStore.ts` (zustand) y `components/ui/NotificationToast.tsx` (montado en `App.tsx`) muestra un toast auto-descartable. Hoy el único origen es el **aviso de moderación** (`MembersSection` "Avisar" → el miembro, si está conectado, ve el toast; además su contador de `warnings` se muestra en la lista). Cadenas en i18n namespace `notifications`.
...
- `EventCreatePage` — **conectado al backend real** desde 2026-06-04. Llama a `POST /api/events` vía `createEvent` de `events.ts`. Mapeo de campos: `desc`→`description`, `date+time`→`date` (ISO combinado), `host`→`topic`, `duration`→`place` (único campo disponible en la entidad `Event`). El form embebido `EventFormSection` de `EventDetailPage` **ya está conectado al backend** (2026-06-11): un cuestionario por evento (opción múltiple o texto), lo crea/borra MOD/ADMIN y lo ven/votan todos los autenticados (anónimos incl., 1 voto por cuenta). Servicios en `events.ts` (`getEventForm`/`createEventForm`/`deleteEventForm`/`voteEventForm`/`respondEventForm`), tipo `ApiEventForm` en `types/api.ts`, y **actualización en vivo por WebSocket** vía `subscribeEventForm(eventId, cb)` de `eventsWS.ts` (topic `/topic/events/:id/form`; el broadcast es público y el componente conserva en local `myVote`/`myResponded`). `wsClient.ts` ganó `offConnect`/`offDisconnect` para suscripciones por-componente sin fugas.
- `DashboardPage` — el **selector de ánimo (las "caras") se oculta a los anónimos** (`useRole().isAnon`); el gato decorativo se mantiene. El seguimiento de ánimo es solo para usuarios registrados.
